...
export default function EditBuyerPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        const fetchSeller = async () => {
            try {
                const response = await fetch(`${API_BASE_URL}/sellers/get-seller-by-id/${resolvedParams.id}`)
                if (!response.ok) {
                    throw new Error('Failed to fetch seller')
                }
                const seller = await response.json()
                setSellerName(seller.seller_name)
                setSellerEmail(seller.seller_email)
                setSellerPhone(seller.seller_phone)
                setSellerStatus(seller.seller_status)
                setSellerAbout(seller.seller_about)
            } catch (error) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const response = await fetch(`${API_BASE_URL}/sellers/update-seller/${resolvedParams.id}`, {
                method: 'PUT',
                headers: {
